...
class UI(object):
  ...
  def setOnlyLED(self, index):
    for i in range(len(self._LEDs)): self.setLED(i, True)
    if index is not None: self.setLED(index, False)
...
class UI_Raspberry(UI):
  def __init__(self):
    # Only for RPi3: set GPIOs to pulldownii
    global rpigpio
    import RPi.GPIO as rpigpio
    rpigpio.setmode(rpigpio.BCM)

    # Layout of GPIOs for Raspberry demo
    self._buttons = [27 ,6 , 17 , 19 , 25]
    self._LEDs = [26 ,5 , 16, 18, 24]
...
class TeachableMachine(object):
  ...
  def visualize(self, classification, svg):
    ...
    classes = ['清除', 'Chrest', 'Teacher', 'Student', 'Other']
    status = '影像:%.1f fps; 總樣本數:%d張; \n辨識類別%7s'%(
    ...
    svg.add(svg.text(status, insert=(25, 45), fill='white', font_size='30'))
